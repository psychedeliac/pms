...
export default function Sidebar({ session }: { session: Session }) {
  ...
  return (
    <aside className="flex min-h-screen w-[256px] shrink-0 flex-col justify-between border-r border-border/5 bg-sidebar-gradient px-5 py-5">
      <div>
        <div className="flex items-center justify-between gap-[10px] p-2 pb-8">
          <div className="flex items-center gap-[10px]">
            ...
            <div>
              <p className="text-[18px] font-medium leading-[28px] tracking-[-0.45px] text-ink">
                concierge
              </p>
              ...
              <>
                ...
                <span
                  className={`pl-3 text-sm ${
                    isActive ? "font-normal text-ink" : "font-light text-muted"
                  }`}
                >
                  {item.label}
                </span>
      ...
      <div className="border-t border-border/5 pt-[21px]">
        <motion.button
          type="button"
          ...
          className="flex w-full cursor-pointer items-center gap-[10px] rounded-xl border border-border/5 bg-surface p-3 backdrop-blur-[10px] transition-colors hover:bg-surface-hover"
        >
          <span className="relative shrink-0">
            ...
            <span className="absolute bottom-0 right-0 size-2.5 rounded-full border-2 border-background bg-[#10b981]" />
          </span>
          <span className="flex-1 text-left">
            <p className="text-xs leading-4 text-ink">{session.name}</p>
            <p className="text-[10px] leading-[15px] font-light text-muted">
              {session.role}
            </p>
          </span>
          <Image
